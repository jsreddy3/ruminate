...
class AgentLoopRunner:
    ...
    async def run_agent_loop(self,
        ...
        Args:
            ...
            session_factory: Function to create database sessions when needed
            session: Optional database session for transactions (legacy support)
        ...
        # Use session factory for database operations if provided
        if session_factory:
            # Create session for initial context building
            async with session_factory() as db_session:
                # Build initial context (database operations)
                messages = await self._build_context_from_service(agent_state, active_thread, db_session)
                
            # Run iterations without holding a persistent DB connection
            final_answer, assistant_msg_id = await self._execute_agent_iterations(
                agent_state, messages, active_thread, session_factory
            )
            
            # Create session for final database updates
            async with session_factory() as db_session:
                # Save process steps
                await self._save_process_steps(agent_state, assistant_msg_id, db_session)
        else:
            # Legacy support for using a single session
    ...
    async def _execute_agent_iterations(self,
                              ...
                              active_thread: List[str],
                              session_or_factory) -> Tuple[str, str]:
        """
        Execute the main agent iteration loop.
        
        Args:
            agent_state: Dictionary containing conversation state
            messages: List of messages for context
            active_thread: List of message IDs in the active thread
            session_or_factory: Either a database session or a session factory
        ...
        # Determine if we're using a session factory or single session
        ...
        while iterations < self.max_iterations and not final_answer:
            ...
            try:
                ...
                # Handle error in action generation (no DB needed)
                ...
                if action_data.get("response_type") == "answer":
                    # Process final answer (needs DB access)
                    if is_factory:
                        ...
                        final_answer, assistant_message_id = await self._process_answer(
                            action_data, agent_state, active_thread, system_msg, iterations, session_or_factory
                        )
                elif action_data.get("response_type") == "action":
                    # Check if this action requires database access
                    # Handle both string actions and dictionary actions with type field
                    action = action_data.get("action")
                    action_type = action.get("type") if isinstance(action, dict) else action
                    needs_db = self._action_needs_database(action_type)
                    
                    if needs_db:
                        # Process tool action with database access
                        if is_factory:
                            async with session_or_factory() as session:
                            ...
                            await self._process_action(
                                action_data, agent_state, working_messages, iterations, session_or_factory
                            )
                    else:
                        # Process tool action without database access
                        await self._process_action(
                            action_data, agent_state, working_messages, iterations, None
                        )
                else:
                    # Handle invalid response type (no DB needed)
                ...
                # Handle any exceptions during iteration (no DB needed)
            ...
            if is_factory:
                async with session_or_factory() as session:
                    if iterations >= self.max_iterations:
                        # Use specialized final iteration approach
                ...
                if iterations >= self.max_iterations:
                    # Use specialized final iteration approach
                    final_answer, assistant_message_id = await self._generate_final_answer(
                        agent_state, active_thread, system_msg, iterations, session_or_factory
                    )
                else:
                    # Handle timeout
                    final_answer, assistant_message_id = await self._handle_timeout(
    ...
    def _action_needs_database(self, action_type: str) -> bool:
        """
        Determine if an action type requires database access.
        
        Args:
            action_type: The type of action being performed
            
        Returns:
            bool: True if the action requires database access, False otherwise
        """
        # List of actions that need database access
        db_actions = [
            "search_document",
        ...
        return action_type in db_actions
